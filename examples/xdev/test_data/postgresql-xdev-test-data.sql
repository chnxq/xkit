-- Description: PostgreSQL test data for xdev current schema
-- Scope:
...
DELETE FROM public.sys_user_org_units WHERE user_id BETWEEN 9001 AND 9010;
DELETE FROM public.sys_users WHERE id BETWEEN 9001 AND 9010;
DELETE FROM public.sys_org_units WHERE id BETWEEN 9101 AND 9110;
DELETE FROM public.sys_user_org_units WHERE user_id BETWEEN 9011 AND 9020;
DELETE FROM public.sys_users WHERE id BETWEEN 9011 AND 9020;
DELETE FROM public.sys_org_units WHERE id BETWEEN 9111 AND 9120;

INSERT INTO public.sys_org_units (
...
INSERT INTO public.sys_user_org_units (user_id, org_unit_id)
VALUES
  (9001, 9101),
  (9002, 9102),
  (9011, 9111),
...
DELETE FROM public.xdev_dev_model_type WHERE id BETWEEN 8101 AND 8199;

-- ---------------------------------------------------------------------------
-- xdev model types
-- ---------------------------------------------------------------------------

INSERT INTO public.xdev_dev_model_type (
...
INSERT INTO public.xdev_dev_model (
  id, created_by, created_at, updated_at, deleted_at, tenant_id,
...
INSERT INTO public.xdev_dev_info (
...
VALUES
  (8921, 1, 'comm-params', '通讯参数', 'COMMUNICATION', 'GLOBAL_SHARED', TRUE, '设备连接、上报与链路保持相关参数', 1),
  (8922, 1, 'device-config-params', '设备配置参数', 'CONTROL', 'DEVICE_GROUP_SHARED', TRUE, '设备采集周期、超时与基础运行配置', 1),
  (8923, 1, 'device-strategy-params', '设备策略控制参数', 'CONTROL', 'DEVICE_GROUP_SHARED', TRUE, '设备告警、重启与策略控制参数', 1),
  (8924, 1, 'business-params', '业务参数', 'ACQUISITION', 'NOT_SHARED', TRUE, '产线业务侧运行指标和阈值参数', 1),
  (8925, 1, 'custom-params', '自定义参数', 'USER_DEFINITION', 'NOT_SHARED', TRUE, '租户自定义扩展参数', 1);
...
INSERT INTO public.xdev_dev_group (
  id, created_by, updated_by, created_at, updated_at, deleted_at, tenant_id,
  status, sort_order, path, group_name, type, is_leaf_node, descript, visible, parent_id
)
VALUES
  (8501, 1, 1, NOW(), NOW(), NULL, 0, 'ON', 1, '/8501/', '平台设备中心', 'FUNCTION', FALSE, '平台总设备目录', TRUE, NULL),
  (8502, 1, 1, NOW(), NOW(), NULL, 0, 'ON', 1, '/8501/8502/', '平台网络组', 'NETWORK', TRUE, '平台网络设备叶子组', TRUE, 8501),
  (8503, 1, 1, NOW(), NOW(), NULL, 0, 'ON', 2, '/8501/8503/', '平台安防组', 'FUNCTION', TRUE, '平台安防设备叶子组', TRUE, 8501),
  (8511, 2, 2, NOW(), NOW(), NULL, 1, 'ON', 1, '/8511/', '租户设备中心', 'FUNCTION', FALSE, '租户总设备目录', TRUE, NULL),
  (8512, 2, 2, NOW(), NOW(), NULL, 1, 'ON', 1, '/8511/8512/', '租户办公组', 'DEPARTMENT', TRUE, '租户办公设备叶子组', TRUE, 8511),
  (8513, 2, 2, NOW(), NOW(), NULL, 1, 'ON', 2, '/8511/8513/', '租户生产组', 'FUNCTION', TRUE, '租户生产设备叶子组', TRUE, 8511),
  (8514, 2, 2, NOW(), NOW(), NULL, 1, 'OFF', 3, '/8511/8514/', '租户环境监测组', 'NETWORK', TRUE, '租户环境设备叶子组', FALSE, 8511);

-- ---------------------------------------------------------------------------
-- xdev group-device relations
-- ---------------------------------------------------------------------------

INSERT INTO public.xdev_dev_group_device (
...
INSERT INTO public.xdev_dev_group_user (
...
INSERT INTO public.xdev_dev_group_org_unit (
...
SELECT setval('xdev_dev_group_id_seq', (SELECT COALESCE(MAX(id), 1) FROM public.xdev_dev_group));
