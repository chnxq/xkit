...
return {{successReturn}}, nil`,
	...
	if !strings.Contains(repoFile, "userRepoCustomList(context.Context, *ent.UserQuery, *paginationv1.PagingRequest) (*paginationv1.PagingRequest, error)") || !strings.Contains(repoFile, "pagingReq, err = custom.userRepoCustomList(ctx, builder, pagingReq)") {
		t.Fatalf("repo file is missing optional list pre-filter hook")
	}
...
func TestRunnerGenerateRepo_RemovesGeneratedSharedHelpersWhenLegacyFilesExist(t *testing.T) {
	root := t.TempDir()
	writeFile(t, filepath.Join(root, "go.mod"), "module example.com/admin\n\ngo 1.24.0\n")
	writeFile(t, filepath.Join(root, "api", "protos", "identity", "v1", "user.proto"), sampleUserProto)
	writeFile(t, filepath.Join(root, "api", "gen", "identity", "v1", "user.pb.go"), sampleUserPB)
	writeFile(t, filepath.Join(root, "internal", "data", "ent", "schema", "user.go"), sampleUserSchema)
	writeFile(t, filepath.Join(root, "internal", "data", "repo", "list_sorting_ext.go"), "package repo\n")
	writeFile(t, filepath.Join(root, "internal", "data", "repo", "tenant_scope_ext.go"), "package repo\n")
	writeFile(t, filepath.Join(root, "internal", "data", "repo", "repo_shared_ext.go"), "// Code generated by xkit.\npackage repo\n")

	cfg := mustLoadConfigFromString(t, sampleUserConfigYAML("example.com/admin"))
	info, err := project.Discover(root, root)
	if err != nil {
		t.Fatalf("discover project: %v", err)
	}

	runner, err := NewProjectRunner(info, cfg, Options{Version: "test"})
	if err != nil {
		t.Fatalf("new runner: %v", err)
	}

	result, err := runner.Generate("repo")
	if err != nil {
		t.Fatalf("generate repo: %v", err)
	}

	sharedPath := filepath.Join(root, "internal", "data", "repo", "repo_shared_ext.go")
	if _, err := os.Stat(sharedPath); err == nil {
		t.Fatalf("repo shared helper file should be removed when legacy helpers exist")
	}
	for _, path := range result.Written {
		if path == sharedPath {
			t.Fatalf("repo shared helper file should not be reported as written when legacy helpers exist")
		}
	}
}
